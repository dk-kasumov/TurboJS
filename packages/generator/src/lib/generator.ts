...
export function generate(parsed, template, isEntryPoint) {
    ...
    walkHtml(parsed.html, {
        ...
        AttributeBinding: {
            enter(node) {
                ...
                current.componentBindings.push({name: node.name, value: code.slice(node.value.start, node.value.end)})
            }
        },
        Text: {
            enter(node) {
        ...
        MustacheTag: {
            enter(node) {
                ...
                const expression = code.slice(node.expression.start, node.expression.end);
                const callee = (node.expression.type === 'CallExpression' && node.expression.callee.name) || `() => ${expression}`;
